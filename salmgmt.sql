...
create table orders(pono int primary key, cgst varchar(20),finished int, foreign key (cgst) references customer(gst) on delete cascade);
create table ordered_pdts1(pono int not null, pid int(5) not null, qty int, drivermob varchar(10), finstage varchar(20), foreign key (pono) references orders(pono) on delete cascade, foreign key (pid) references products(pid) on delete cascade, primary key(pono,pid));
